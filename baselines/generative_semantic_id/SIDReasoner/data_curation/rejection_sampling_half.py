...
def select_dataset_half(source: Any, dataset_half: str) -> Any:
    if dataset_half == "all":
        return source
    half_index = {"first": 0, "second": 1}.get(dataset_half)
    if half_index is None:
        raise ValueError(f"unsupported dataset half: {dataset_half}")
    base_size, remainder = divmod(len(source), 2)
    start = half_index * base_size + min(half_index, remainder)
    size = base_size + int(half_index < remainder)
