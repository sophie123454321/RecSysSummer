...
The accepted rows preserve the source schema, but replace ``reasoning_path``
with the reasoning generated by the checkpoint. They can be uploaded as
``<Category>_reasoning/train`` and consumed directly by
...
DEFAULT_SOURCE_HF_REPO = "yufan/recsys-genrec-dataset-refresh-gpt5.4"
...
def run_signature(args: argparse.Namespace, source_config: str) -> str:
    payload = {
        ...
        "dataset_third": args.dataset_third,
...
def extract_reasoning_path(tokenizer: Any, response_ids: list[int], sampled_length: int) -> str:
    reasoning = tokenizer.decode(
        response_ids[:sampled_length],
    ...
    if not reasoning:
        raise RuntimeError("checkpoint generated an empty reasoning_path")
    return reasoning
    ...
    for cutoff in RANK_CUTOFFS:
        ...
        for source_index in range(len(source)):
            ...
            if isinstance(rank, int) and rank <= cutoff:
                ...
                if not isinstance(reasoning_path, str) or not reasoning_path.strip():
                    raise RuntimeError(
                        f"accepted row {source_index} has no checkpoint-generated reasoning_path"
                    )
...
def select_dataset_third(source: Any, dataset_third: str) -> Any:
    if dataset_third == "all":
        return source
    third_index = {"first": 0, "second": 1, "third": 2}.get(dataset_third)
    if third_index is None:
        raise ValueError(f"unsupported dataset third: {dataset_third}")
    base_size, remainder = divmod(len(source), 3)
    start = third_index * base_size + min(third_index, remainder)
    size = base_size + int(third_index < remainder)
    return source.select(range(start, start + size))
...
def main() -> None:
    ...
    source = select_dataset_third(source, args.dataset_third)
    if args.num_shards > 1:
        source = source.shard(
            num_shards=args.num_shards,
        ...
        for task_batch in tqdm(
            ...
            for output in reasoning_outputs:
                ...
                reasoning_paths.append(
                    extract_reasoning_path(tokenizer, response_ids, sampled_length)
                )
            ...
            for (key, source_index, row), sample_beams, reasoning_path in zip(
                task_batch,
                beams,
                reasoning_paths,
            ):
